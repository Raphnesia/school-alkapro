...
import { facilityService, SubFacility } from '@/services/facilityService'
import { ErrorMessage } from '@/components/ErrorMessage'

interface SubFacilityDetailPageProps {
  params: Promise<{
    parentSlug: string
    slug: string
  }>
...
const SubFacilityDetailPage: React.FC<SubFacilityDetailPageProps> = ({ params }) => {
  const [facility, setFacility] = React.useState<SubFacility | null>(null)
  ...
  React.useEffect(() => {
    ...
    const fetchFacility = async () => {
      try {
        ...
        const data = await facilityService.getSubFacility(parentSlug, slug)
        setFacility(data)
      } catch (err) {
        setError(err instanceof Error ? err.message : 'Terjadi kesalahan saat mengambil data')
        console.error('Error fetching sub-facility detail:', err)
      } finally {
        setIsLoading(false)
      }
    }
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="bg-white shadow-sm">
        <div className="container mx-auto px-4 py-4">
             ...
             <span>/</span>
             <Link href={`/fasilitas/${parentSlug}`} className="hover:text-blue-600 capitalize">
               {parentSlug.replace('-', ' ')}
             </Link>
             <span>/</span>
      ...
      <div className="flex-1">
        <div className="container mx-auto px-4 py-8">
          {/* Back Button */}
                     <div className="mb-6">
             <Link 
               href={`/fasilitas/${parentSlug}`}
               className="inline-flex items-center text-blue-600 hover:text-blue-700"
             >
               <ArrowLeft size={16} className="mr-2" />
               Kembali ke {parentSlug.replace('-', ' ')}
             </Link>
           </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div>
              ...
              {/* Action Buttons */}
                 ...
                 >
                   Lihat Semua {parentSlug.replace('-', ' ')}
                 </Link>
...
export default SubFacilityDetailPage 