// src/components/PimpinanSMPCard.tsx
// Komponen untuk menampilkan kartu profil pimpinan SMP

import React from 'react';
import Image from 'next/image';
import { PimpinanSMP } from '@/services/pimpinanSMPService';
import { ScrollReveal } from './ScrollReveal';
import { useI18n } from '@/hooks/useI18n';

interface PimpinanSMPCardProps {
  pimpinan: PimpinanSMP;
  layout: 'left' | 'right';
}

export function PimpinanSMPCard({ pimpinan, layout }: PimpinanSMPCardProps) {
  const { t } = useI18n();
            ...
            <div className="w-full lg:hidden flex flex-col items-center text-center py-6 pb-12" style={{fontFamily: 'Ubuntu, sans-serif'}}>
              ...
              <ScrollReveal direction="up" delay={400} duration={600}>
                <div className="px-4 text-left">
                  ...
                  {education && (
                    <p className="text-sm text-gray-600 mb-2">
                      <strong>{t('pimpinan.pendidikan')}:</strong> {education}
                    </p>
                  )}
                  
                  {experience && (
                    <p className="text-sm text-gray-600 mb-4">
                      <strong>{t('pimpinan.pengalaman')}:</strong> {experience}
                    </p>
                  )}
                </div>
              </ScrollReveal>
    ...
    <>
      ...
      <ScrollReveal direction="left" delay={300} duration={700}>
        <div className="flex items-center justify-center p-4">
                      ...
                      <div className="max-w-none pr-4">
                        ...
                        {education && (
                          <p className="text-sm text-gray-700 mb-2">
                            <strong className="text-black">{t('pimpinan.pendidikan')}:</strong> {education}
                          </p>
                        )}
                        
                        {experience && (
                          <p className="text-sm text-gray-700 mb-4">
                            <strong className="text-black">{t('pimpinan.pengalaman')}:</strong> {experience}
                          </p>
                        )}
                      </div>
                    </div>
          ...
          {/* Green Bar - Posisi di bottom dengan height yang menyesuaikan */}
          <ScrollReveal direction="up" delay={600} duration={500}>
            <div className="w-full h-8 lg:h-20 bg-green-600 flex justify-center absolute bottom-0 mx-0 z-50">
              {layout === 'left' ? (
                <>
